#!/usr/bin/env bash
# Bootstrap signing CarenceScan — cert + profil App Store via Fastlane + API Apple
set -euo pipefail
...
BUNDLE_APP="${BUNDLE_ID_APP:-com.carencescan.app}"
OUT_DIR="${GITHUB_WORKSPACE}/output/signing-bundle"
API_JSON="${RUNNER_TEMP}/asc_api_key.json"
KEYCHAIN_PATH="${RUNNER_TEMP}/carencescan-bootstrap.keychain-db"
